...
Phase 2 已于 2026-07-15 在真实 Neo4j、Ollama 和固定 200 道菜数据上完成验收：

- 默认测试：`126 passed, 2 deselected`，无 TestClient/httpx 弃用警告
- 真实 Neo4j/Ollama integration：`2 passed, 126 deselected`
- 新 200 道菜版本构建、激活、dry-run、验证回滚和最终版本恢复均成功
- Neo4j：6 个命名唯一约束、1 个 Recipe 版本索引；两个保留版本各 200 道菜
- 清理 dry-run：活动/上一版本均受保护，缺失必保版本、候选、无效目录和 graph-only 版本均为 `0`
- 固定 50 条查询：Recall@5 `0.90`，硬条件违规 `0`，失败 `0`
- 搜索 P50 / P95：`14.04ms / 271.83ms`
- 回答首字 P50 / P95：`529.56ms / 543.11ms`
- API cold-start：`5/5` 成功，ready P50 / P95 `1711.49ms / 2308.97ms`，首次搜索 P50 / P95 `2.88ms / 3.18ms`
- `/health`、`/ready`、`/`、`/static/app.js`：均为 HTTP `200`
- Ruff、`pip check` 与 Git 差异检查通过
...
报告包含 Recall@5、硬条件违规数、搜索 P50/P95、首字 P50/P95、样本数、未命中案例、按意图汇总、降级组件和安全失败类别。任一验收门槛失败或必需请求失败时，脚本仍写报告并返回非零退出码。2026-07-15 的真实报告中四项门槛全部为 `true`。
